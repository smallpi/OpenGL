 ...
 *		可以只传送一遍uniform数据，在多个shader之间共用
 *		uniform block块采用std140标准进行内存布局
 *		其他布局标准还有
 *			shared
 *			std140
 *			packed
 *
 *		Shader类中添加uniformBlockBinding方法，
 *		将Shader中uniform block块绑定到binding point
 **/
...
void onInit(void){
	...
	//  
	//	可以查询Uniform数据的最大使用数量
	//	glGetIntegerv(GL_MAX_VERTEX_UNIFORM_COMPONENTS,&a);
	//	通过使用uniform buffer可以获得更多的使用数量
	...
	//shader绑定在bindingPoint
	shader.uniformBlockBinding("Matrices",0);
	lampShader.uniformBlockBinding("Matrices",0);
	//UBO绑定到bindingPoint
...
void onDraw(GLFWwindow* window){
	...
	glBindBuffer(GL_UNIFORM_BUFFER,UBO);
	projectMat.transpose();
	glBufferSubData(GL_UNIFORM_BUFFER,0,sizeof(mat4),projectMat.data());
	viewMat.transpose();
	glBufferSubData(GL_UNIFORM_BUFFER,sizeof(mat4),sizeof(mat4),viewMat.data());
	glBindBuffer(GL_UNIFORM_BUFFER,0);
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(640, 480, "OpenGL's road - Uniform buffer objects", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
