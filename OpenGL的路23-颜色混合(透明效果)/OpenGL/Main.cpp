...
void onInit(void){
    ...
    glBindVertexArray(0);
    // 创建纹理并加载纹理
    textureDiffuse = Texture::loadTexture(GL_TEXTURE_2D,"./images/transparentWindow.png");
    textureSpecular = Texture::loadTexture(GL_TEXTURE_2D,"./images/transparentWindow.png");
...
void onDraw(GLFWwindow* window){
	...
	map<float,int> winMap;
	for(int i=0; i<8; i++){
		float distance = sqrt(
			(cubePosition[i][0] - cameraPos[0])*(cubePosition[i][0] - cameraPos[0])
			+ (cubePosition[i][1] - cameraPos[1])*(cubePosition[i][1] - cameraPos[1])
			+ (cubePosition[i][2] - cameraPos[2])*(cubePosition[i][2] - cameraPos[2]) );
		winMap[distance] = i;
	}

	for(map<float,int>::reverse_iterator iter = winMap.rbegin(); iter != winMap.rend(); iter++){
		mat4 transform(1.0f);
        mat::translate(transform, cubePosition[iter->second][0], cubePosition[iter->second][1], cubePosition[iter->second][2]);
		mat::scale(transform,2.0f,2.0f,2.0f);
        shader.setMat4v("transform",transform.data(),true);
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(640, 480, "OpenGL's road - Blending", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
