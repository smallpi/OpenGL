...
unsigned int modelVAO;	// 模型 
unsigned int cubeVAO;	// 正方体灯
unsigned int modelVBO;	// 存储模型
unsigned int cubeVBO;	// 存储正方体
...
void onInit(void){
	...
	// 可视化法向量的shader
	normalShader.compile(GL_VERTEX_SHADER,"./normal_vert.glsl",
		GL_GEOMETRY_SHADER,"./normal_geom.glsl",
		GL_FRAGMENT_SHADER,"./normal_frag.glsl");
    // 初始化光源正方体的shader
...
void onDraw(GLFWwindow* window){
	...
	normalShader.use();
	normalShader.setMat4v("view",viewMat.data(),true);
    normalShader.setMat4v("project",projectMat.data(),true);
	normalShader.setMat4v("transform",transform.data(),true);
    for(int i=0; i<models.size(); i++){
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D,models[i].diffuseTexture);
