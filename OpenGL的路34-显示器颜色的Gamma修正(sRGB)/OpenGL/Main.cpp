 ...
 *	description: 
 *		显示器默认显示会比较暗
 *		可以通过Gamma修正，即对颜色 乘幂 1/gamma
 *		通常gamma值为2.2
 *		有两种gamma修正的方法
 *			方法一：通过GL_ENABLE(GL_FRAMEBUFFER_SRGB); // 这种方法在每次fragmentshader输出都会修正
 *			方法二：自己在FragmentShader实现Gamma修正
 *		本例程中采用第一种方式，通过按键G启动和关闭GAMMA修正
 *		FramentShader中有Gamma修正的实现代码，已经注释起来。
 *
 *		有时候我们的贴图纹理已经是在sRGB空间了
 *		此时可以使用GL_SRGB或GL_SRGB_ALPHA内部格式对贴图纹理进行修正
 *		使得贴图纹理回到线性空间。
 *		一般代表颜色的纹理贴图是sRGB空间的
 *		而代表光照属性的纹理贴图是线性空间的
 *
 *		在进行Gamma时同时需要考虑光照的attenuation问题
 *		一般Gamma修正时采用线性的attenuation
 *		没有Gamma修正时采用二次的attenuation
 **/
