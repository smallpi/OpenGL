...
void onInit(void){
    ...
    glVertexAttribPointer(0,3,GL_FLOAT,GL_FALSE,6*sizeof(float),(void*)0);  // 此时间距变化了
	//另VBO的颜色部分数据指向shader的location=0
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1,3,GL_FLOAT,GL_FALSE,6*sizeof(float),(void*)(3*sizeof(float)));  // 此时间距变化了
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(640, 480, "OpenGL's road - Colored rectangle", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
