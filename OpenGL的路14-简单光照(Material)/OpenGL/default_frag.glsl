...
void main(void){
    ...
    vec3 ambient = ambientIntensity * light.ambient * material.ambient;

    vec3 normal = normalize(vNormal);
    vec3 lightDir = normalize(light.pos - vFragPos);
    vec3 diffuseIntensity = vec3(1.0f);
    float diffuseStrength = max(dot(normal, lightDir), 0.0f);
    vec3 diffuse = diffuseIntensity * diffuseStrength * light.diffuse * material.diffuse;

    vec3 cameraDir =  normalize(cameraPos - vFragPos);
    vec3 reflectDir = reflect(-lightDir, normal);
    vec3 specularIntensity = vec3(0.5f);
    float specularStrength = pow(max(dot(cameraDir, reflectDir), 0.0f), material.shininess);
    vec3 specular = specularIntensity * specularStrength * light.specular * material.specular;
