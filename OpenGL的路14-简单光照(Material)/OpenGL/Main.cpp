...
void onDraw(GLFWwindow* window){
    ...
    mat4 transform(1.0f);
    shader.setMat4v("transform",transform.data());

    // 向shader传递物体材质颜色(黄铜）和光照颜色
    shader.setVec3("material.ambient",0.329412f,0.223529f,0.027451f);
    shader.setVec3("material.diffuse",0.780392f,0.568627f,0.113725f);
    shader.setVec3("material.specular",0.992157f,0.941176f,0.807843f);
    shader.setFloat("material.shininess",0.21794872f * 128.0f);
    shader.setVec3v("light.pos",lightPos);
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(640, 480, "OpenGL's road - A green rubber box-Phone shading", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
