 ...
 *		进行简单的光照渲染
 *		给每个顶点添加了法向量数据
 *		在fragment shader(View Space)计算光照
 *
 **/
...
void onDraw(GLFWwindow* window){
    ...
    // view space计算光照cameraPos为(0,0,0)
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(640, 480, "OpenGL's road - A lamp and a lighted box-Phone shading(on view space)", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
