...
void main(void){
    ...
    // 反射部分
    // 可以通过通过 Reflection map来控制反射的部分。
    // 入射向量：眼睛看下fragPos的向量
    vec3 I = normalize(vFragPos - cameraPos);
    // 出射向量：cubemap的采样向量
    vec3 R = reflect(I, normalize(vNormal));
    vec3 reflectColor = vec3(texture(skyboxTexture, R));


    // 折射部分
    // 从空气到玻璃的折射率
    // float ratio = 1.0f / 1.52f;
    // vec3 refractVector = refract(I, normalize(vNormal), ratio);
    // vec3 refractColor = vec3(texture(skyboxTexture, refractVector));

    finalColor = mix(finalColor, reflectColor, 0.3f);
