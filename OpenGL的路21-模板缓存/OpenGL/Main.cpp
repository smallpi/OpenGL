...
void onInit(void){
    ...
    glClearDepth(1.0f);
    //glClearStencil(0);  //这是stencilBuffer的默认值
    ...
    singleColorShader.compile(GL_VERTEX_SHADER,"./default_singleColor_vert.glsl",
        GL_FRAGMENT_SHADER,"./default_singleColor_frag.glsl");
}
...
void onDraw(GLFWwindow* window){
    ...
    glStencilFunc(GL_ALWAYS,1,0xFF); // 模板测试总是可以通过，比较值为1,0xFF表示所有位都进行比较
    glStencilMask(0xFF); // 0xFF表示允许写入模板缓存，若0x00则不允许写入模板缓存，实际是与的意思
    ...
    glStencilMask(0x00);
    glDisable(GL_DEPTH_TEST);  // 不进行深度测试，总是能画出来

    singleColorShader.use();
    singleColorShader.setMat4v("view",viewMat.data(),true);
    singleColorShader.setMat4v("project",projectMat.data(),true);
    for(int i=0; i<5; i++){
        mat4 transform(1.0f);
        mat::scale(transform,1.1f,1.1f,1.1f);
        mat::translate(transform, cubePosition[i][0], cubePosition[i][1], cubePosition[i][2]);
        singleColorShader.setMat4v("transform",transform.data(),true);
        glDrawArrays(GL_TRIANGLES,0,36);
    }
    glStencilMask(0xFF);
    glEnable(GL_DEPTH_TEST);
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(640, 480, "OpenGL's road - Outline with Stencil Test", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
