...
void main(void){
    ...
    vec3 finalColor = calcDirLight(dirLight, fs_in.normal, fs_in.fragPos, cameraPos);
    finalColor += calcPointLight(pointLight, fs_in.normal, fs_in.fragPos, cameraPos);
...
vec3 calcDirLight(DirLight light, vec3 normal, vec3 fragPos, vec3 cameraPos){
    ...
    vec3 ambient = ambientIntensity * light.ambient * vec3(texture(material.diffuse, fs_in.texCoord));

    vec3 lightDir = normalize(light.dir);
    vec3 diffuseIntensity = vec3(1.0f);
    float diffuseStrength = max(dot(normal, lightDir), 0.0f);
    vec3 diffuse = diffuseIntensity * diffuseStrength * light.diffuse * vec3(texture(material.diffuse, fs_in.texCoord));
    ...
    vec3 specular = specularIntensity * specularStrength * light.specular * vec3(texture(material.specular, fs_in.texCoord));
