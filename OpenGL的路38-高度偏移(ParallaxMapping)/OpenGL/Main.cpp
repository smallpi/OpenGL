...
void onInit(void){
    ...
    unsigned char* data = stbi_load("./images/brickwall.jpg",&width,&height,&nChannel,0);
    if(data){
        //第7个参数时图像的颜色格式
        //第3个参数才是需要存储的颜色格式
	...
	data = stbi_load("./images/brickwall_normal.jpg",&width,&height,&nChannel,0);
	if(data){
		glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,width,height,0,GL_RGB,GL_UNSIGNED_BYTE,data);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);
	...
	data = stbi_load("./images/brickwall_disp.jpg",&width,&height,&nChannel,0);
	if(data){
		glTexImage2D(GL_TEXTURE_2D,0,GL_RGB,width,height,0,GL_RGB,GL_UNSIGNED_BYTE,data);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);
	...
	shaderList.push_back(Shader::createShader(GL_GEOMETRY_SHADER,"./default_geom.glsl"));
...
void onDraw(GLFWwindow* window){
	...
	// 光照
	glUniform3fv(glGetUniformLocation(shaderId,"ligthPos"),1,lightPos);
	...
	glBindTexture(GL_TEXTURE_2D,heightTexture);
	glUniform1i(glGetUniformLocation(shaderId,"heightTexture"),2);
...
int main(int argc, char** argv){
    ...
    // 创建windows mode的窗口
    // 并创建OpenGL环境
    window = glfwCreateWindow(screen_width, screen_height, "OpenGL's road - Parallax Mapping", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
