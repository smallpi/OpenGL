...
void main(void){
    ...
    gTBN = transpose(mat3(tangle, bitangle, N)); // 要转置,glsl中矩阵列优先存储    
    gNormal = normal;

    gTangentLightPos = gTBN * lightPos;
    gTangentCameraPos = gTBN * cameraPos;

    gTexCoord = vTexCoord[0];
    gFragPos = vFragPos[0];
    gTangentFragPos = gTBN * vFragPos[0];
    gl_Position = gl_in[0].gl_Position;
    EmitVertex();

    gTexCoord = vTexCoord[1];
    gFragPos = vFragPos[1];
    gTangentFragPos = gTBN * vFragPos[1];
    gl_Position = gl_in[1].gl_Position;
    EmitVertex();

    gTexCoord = vTexCoord[2];
    gFragPos = vFragPos[2];
    gTangentFragPos = gTBN * vFragPos[2];
