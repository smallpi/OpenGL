...
void main(void){
    ...
    if(gl_FragCoord.x > SCREEN_WIDTH / 2){
        oColor = vec4(1.0f - finalColor, 1.0f);
    }else{
        oColor = vec4(finalColor, 1.0f);
    }


    //  gl_FragDepth (float)可以写入深度值，默认取值为gl_FragCoord.z
    //      可以通过layout(depth_xxx)限制gl_FragDepth的修改 (OpenGL 4.2及之后)
    //      layout(depth_any) gl_FragDepth可以修改成任意值
    //      layout(depth_greater) gl_FragDepth只能比gl_FragCoord.z大
    //      layout(depth_less) gl_FragDepth只能比gl_FragCoord.z小
    //      layout(depth_unchanged) gl_FragDepth只能比gl_FragCoord.z相同
